  ...
  namespace manipulation {
    namespace pathPlanner {
          ...
          OptimizationData& d, const Edges_t& transitions) const
      {
        ...
        {
          Configuration_t* qptr = new Configuration_t (d.q1);
          ConfigurationPtr_t q (qptr);
          pv.push_back(q);
        }
        for (std::size_t i = 0; i != transitions.size()-1; ++i)
        {
          Configuration_t* qptr = new Configuration_t (d.waypoint.col (i));
          ConfigurationPtr_t q (qptr);
          pv.push_back(q);
        }
        {
          Configuration_t* qptr = new Configuration_t (d.q2);
          ConfigurationPtr_t q (qptr);
          pv.push_back(q);
        }
        return pv;
          ...
          while (! transitions.empty()) {
            ...
            if (buildOptimizationProblem (optData, transitions)) {
              if (solveOptimizationProblem (optData)) {
                core::Configurations_t path = buildPath (optData, transitions);
